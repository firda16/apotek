...
<body>
    ...
    <table>
        <tr>
            <td><strong>Total Stok Saat Ini</strong></td>
            <td class="text-right">{{ $currentStock->sum('available_stock') }}</td>
        </tr>
        <tr>
            <td><strong>Total Stok Masuk</strong></td>
            <td class="text-right">{{ $stockInSummary->sum('total_quantity') }}</td>
        </tr>
        <tr>
            <td><strong>Total Stok Keluar</strong></td>
            <td class="text-right">{{ $stockOutSummary->sum('total_quantity') }}</td>
        </tr>
    </table>
        ...
        <tbody>
            @forelse ($currentStock as $i => $row)
                <tr>
                    ...
                    <td class="text-right">{{ $row['available_stock'] }}</td>
    ...
    <table>
        ...
        <tbody>
            @forelse ($stockIn as $i => $item)
                <tr>
                    <td class="text-center">{{ $i + 1 }}</td>
                    <td class="text-center">{{ \Carbon\Carbon::parse($item->purchase->created_at)->format('d-m-Y') }}
                    </td>
