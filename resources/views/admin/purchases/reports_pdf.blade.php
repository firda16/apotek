...
<head>
    ...
    <style>
        ...
        th, td {
            ...
            padding: 6px;
            font-size: 10px;
            vertical-align: middle; /* Ensures rowspan content is centered vertically */
        }
        th {
        ...
        <tbody>
            ...
            @forelse($pembelians as $purchase)
                ...
                <tr>
                    ...
                    @if ($loop->first)
                        <td class="text-center" rowspan="{{ $purchase->purchaseItems->count() }}">{{ \Carbon\Carbon::parse($purchase->created_at)->format('d-m-Y') }}</td>
                        <td rowspan="{{ $purchase->purchaseItems->count() }}">{{ $purchase->invoice_number ?? '-' }}</td>
                        <td rowspan="{{ $purchase->purchaseItems->count() }}">{{ $purchase->supplier->name ?? '-' }}</td>
                        <td class="text-center" rowspan="{{ $purchase->purchaseItems->count() }}">{{ $purchase->payment_method ?? '-' }}</td>
                    @endif
